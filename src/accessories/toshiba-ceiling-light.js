...
class ToshibaCeilingLightAccessory extends BaseAccessory {
  ...
  async normal_on_set(val) {
    if ( val ) {
      this.set_states({mode : 'normal'});
    } else {
      this.set_states({mode : 'off'});
    }
  }
  ...
  async oyasumi_on_set(val) {
    if ( val ) {
      this.set_states({mode : 'oyasumi-assist'});
    } else {
      this.set_states({mode : 'off'});
    }
  }
  ...
  async nightlight_on_set(val) {
    if ( val ) {
      this.set_states({mode : 'night-light'});
    } else {
      this.set_states({mode : 'off'});
    }
  }
  ...
  static #hsl2rgb(h, s, l) {
    ...
    return {
      r : Math.round(10 * f(0)), g : Math.round(10 * f(8)),
      b : Math.round(10 * f(4)),
    };
  }
